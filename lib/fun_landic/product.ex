defmodule FunLandic.Product do
  ...
  def map(%Product{val: val}, function) do
    %Product{val: function.(val) |> assert_value_is_number}
  end

  def apply_with(%Product{val: fun}, %Product{val: val}) do
    %Product{val: fun.(val) |> assert_value_is_number}
  end
  ...
  """
  def wrap(val) when is_number(val) or is_function(val) do
    %Product{val: val}
  end
